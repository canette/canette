...
type BuildConfig struct {
	...
	TrivyImage         string // e.g. "aquasec/trivy:0.51.0"
}
...
// ScanJobName returns the deterministic K8s Job name for a scan job.
func ScanJobName(deploymentID string) string {
	short := deploymentID
	if len(short) > 8 {
		short = short[:8]
	}
...
// scanScript is the container script that runs Trivy, emits structured log lines,
// and writes the SBOM as base64 so the builder can read it back from logs.
const scanScript = `set -eu
IMAGE_REF="${IMAGE_REF}"
FAIL_SEVERITY="${FAIL_SEVERITY:-CRITICAL}"
...
# Parse counts per severity from the findings JSON using awk (no jq required).
...
// ScanJob constructs a batchv1.Job that runs Trivy against a pushed image.
func ScanJob(deploymentID, imageRef string, cfg BuildConfig) *batchv1.Job {
	jobName := ScanJobName(deploymentID)
	ttl := int32(600)
	backoff := int32(0)
