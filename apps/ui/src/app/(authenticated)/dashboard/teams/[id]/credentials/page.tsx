...
export default function CredentialsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleAddCred(e: React.FormEvent) {
    ...
    setCredError(""); setAddingCred(true)
    try {
      const cred = await api.teams.createCredential(teamId, {
        name: credName.trim(), provider: credProvider, type: credType, value: credValue.trim(),
        sshKnownHosts: credType === "ssh_key" && credKnownHosts.trim() ? credKnownHosts.trim() : undefined,
      })
      setCredentials((prev) => [cred, ...prev])
      setCredName(""); setCredValue(""); setCredKnownHosts(""); setCredShowValue(false)
    } catch (e: unknown) {
  ...
  async function handleUpdateCred(e: React.FormEvent, id: string) {
    ...
    setEditCredError(""); setSavingCred(true)
    try {
      ...
      setEditingCredId(null); setEditCredValue("")
    } catch (e: unknown) {
  ...
  async function handleConnectGithubApp() {
    ...
    try {
      ...
      if (installations.length > 0) { setLinkableInstallations(installations); setLinkDialogOpen(true); return }
      const result = await api.githubApp.getInstallUrl(teamId)
      if (!result.available || !result.url) { setGithubAppNotice("Per-Team GitHub App support is not configured on this instance. Ask your admin to set it up."); return }
      window.location.href = result.url
    } catch { setGithubAppNotice("Failed to get GitHub App install URL. Please try again.") }
    finally { setConnectingGithubApp(false) }
  }

  async function handleConnectNewAccount() {
    setLinkDialogOpen(false); setConnectingGithubApp(true)
    try {
      const result = await api.githubApp.getInstallUrl(teamId)
      if (!result.available || !result.url) { setGithubAppNotice("Per-Team GitHub App support is not configured on this instance. Ask your admin to set it up."); return }
      window.location.href = result.url
    } catch { setGithubAppNotice("Failed to get GitHub App install URL. Please try again.") }
    finally { setTimeout(() => setConnectingGithubApp(false), 2000) }
  }
  ...
  async function handleLinkInstallation(installationId: string) {
    ...
    } catch (e) { setGithubAppNotice(e instanceof Error ? e.message : "Failed to link installation.") }
                ...
                <div key={cred.id}>
                  ...
                  <div className="flex items-center gap-4 px-6 py-3 group">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100">
                      ...
                      <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-muted-foreground hover:text-destructive"
                        onClick={() => handleDeleteCred(cred.id)}>×</Button>
                    </div>
                  </div>
                  {editingCredId === cred.id && (
                    <form onSubmit={(e) => handleUpdateCred(e, cred.id)}
                      ...
                      {cred.type === "ssh_key" ? (
                        <textarea className="min-h-[140px] rounded-md border border-input bg-background px-3 py-2 text-xs font-mono resize-y"
                        ...
                        <div className="flex items-center gap-2">
                          <Input type={editCredShowValue ? "text" : "password"} placeholder="ghp_..."
                            value={editCredValue} onChange={(e) => setEditCredValue(e.target.value)}
          ...
          <form onSubmit={handleAddCred} className="px-6 py-4 flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <Input id="cred-name" placeholder="e.g. GitHub (work)" value={credName} onChange={(e) => setCredName(e.target.value)} />
              </div>
              <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex rounded-md border border-border overflow-hidden w-fit">
                <button type="button" onClick={() => { setCredType("pat"); setCredValue("") }}
                  className={cn("px-4 py-1.5 text-sm transition-colors",
                    credType === "pat" ? "bg-foreground text-background font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted")}>
                  PAT
                </button>
                <button type="button" onClick={() => { setCredType("ssh_key"); setCredValue(""); setCredShowValue(false) }}
                  className={cn("px-4 py-1.5 text-sm transition-colors border-l border-border",
                    credType === "ssh_key" ? "bg-foreground text-background font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted")}>
                  SSH Key
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="cred-value">{credType === "pat" ? "Token" : "Private key"}</Label>
              {credType === "ssh_key" ? (
                <textarea id="cred-value"
                  className="min-h-[140px] rounded-md border border-input bg-background px-3 py-2 text-xs font-mono resize-y"
                  placeholder={"-----BEGIN OPENSSH PRIVATE KEY-----\n...\n-----END OPENSSH PRIVATE KEY-----"}
                  value={credValue} onChange={(e) => setCredValue(e.target.value)} spellCheck={false} autoComplete="off" />
              ) : (
                <div className="flex items-center gap-2">
                  <Input id="cred-value" type={credShowValue ? "text" : "password"} placeholder="ghp_..."
                    value={credValue} onChange={(e) => setCredValue(e.target.value)} className="font-mono text-xs" />
                  <button type="button" onClick={() => setCredShowValue((v) => !v)}
                    className="text-muted-foreground hover:text-foreground shrink-0" tabIndex={-1}
            ...
            {credType === "ssh_key" && (
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="cred-known-hosts">Known hosts <span className="ml-2 text-xs text-muted-foreground font-normal">optional</span></Label>
                <textarea id="cred-known-hosts"
                  ...
                  value={credKnownHosts} onChange={(e) => setCredKnownHosts(e.target.value)} />
              </div>
            )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            ...
            {!team.isPersonal && <span><br />All team members can use the connected credentials when adding apps.</span>}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {githubAppNotice && (
                  ...
                  <div className="flex items-center gap-4 px-6 py-3 group">
                    ...
                    {cred.installationId && cred.connectedByUserId === session?.user?.id ? (
                      <a href={`https://github.com/settings/installations/${cred.installationId}`}
                        ...
                        title="Configure repo access on GitHub">Configure ↗</a>
                    ) : <span className="w-20" />}
                    <span className="text-xs text-muted-foreground w-20 text-right">{timeAgo(cred.createdAt)}</span>
                    <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100"
                      onClick={() => handleDeleteCred(cred.id)} title="Remove installation">×</Button>
                  </div>
                </div>
              ))}
              <Separator />
          ...
          <div className="px-6 py-4">
            <Button size="sm" variant="outline" onClick={handleConnectGithubApp} disabled={connectingGithubApp}>
      ...
      <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Link your existing Github App to this team or connect a new GitHub account or org.</DialogDescription>
          </DialogHeader>
          ...
          <div className="px-6 pb-6 pt-2 border-t border-border/50 mt-2">
            <Button size="sm" variant="ghost" className="text-muted-foreground hover:text-foreground" onClick={handleConnectNewAccount}>
              <GitHubIcon size={15} />
