...
// HTTP Basic Auth with any username for scripts/CI). A single shared
// password, not individual accounts. Unlike custom hostnames this is an
...
export function PasswordGateManager({ appId }: { appId: string }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      const status = await api.passwordGate.enable(appId, password)
      setGate(status)
      setDialogOpen(false)
    } catch (e: unknown) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-xs text-muted-foreground">
        Requires a password before the app&apos;s public URL is reachable. Changes take effect on
        the next deployment.
      </p>
      ...
      {gate.enabled ? (
        <div className="flex items-center gap-3 rounded-md border px-4 py-3">
          <div className="flex-1 min-w-0">
            <span className="text-sm text-muted-foreground">Password protection is enabled</span>
          </div>
          <Button size="sm" variant="outline" onClick={openDialog}>Change password</Button>
      ...
      {/* Enable/change password dialog */}
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{gate.enabled ? "Change password" : "Enable password protection"}</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4 px-6 pb-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button size="sm" onClick={handleSave} disabled={saving || !password}>
                {saving ? "Saving…" : "Save"}
              </Button>
            </div>
          </div>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col gap-4 px-6 pb-6">
            <p className="text-sm text-muted-foreground">
              The public URL will become reachable without a password.
            </p>
