...
apps:
  - name: My App                 # display name — required
    ...
    deployment_type: web       # "web" (default) or "private" — see below
    port: 3000
...
- **`deployment_type`** controls whether a public URL is created. `web` (default) creates a `Deployment` + `Service` + `HTTPRoute`. `private` creates only `Deployment` + `Service` — the app is reachable inside the cluster at `<slug>.<namespace>.svc.cluster.local` but has no public URL. Use `private` for databases, internal APIs, and background services. See [Apps — Visibility](/docs/concepts/apps#visibility-deployment-type).
