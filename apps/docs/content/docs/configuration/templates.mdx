...
- **Volumes are not supported in templates.** `app_volumes` (PVC / emptyDir / configmap mounts) are always configured after the app exists, on its settings page — there's no `volumes` field in either a template or `canette.yaml`. Add volumes to each app individually after running the wizard.
