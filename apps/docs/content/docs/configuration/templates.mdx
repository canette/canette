...
apps:
  - name: My App                 # display name — required
    ...
    deployment_type: web       # "web" (default), "private", or "cronjob" — see below
    port: 3000                 # not used for deployment_type: cronjob

    # Only for deployment_type: cronjob — a standard 5-field cron expression
    # or a predefined schedule (@hourly, @daily, @weekly, @monthly, @yearly, @midnight)
    # schedule: "0 2 * * *"

    # Env vars (plaintext, shown and editable in the wizard)
    # PORT is reserved — canette injects it automatically from `port` above,
    # so it cannot be set here or as a secret.
    env:
      KEY: value
...
- **`deployment_type`** controls what gets created. `web` (default) creates a `Deployment` + `Service` + `HTTPRoute`. `private` creates only `Deployment` + `Service` — the app is reachable inside the cluster at `<slug>.<namespace>.svc.cluster.local` but has no public URL. `cronjob` creates a `CronJob` instead of a `Deployment` and requires `schedule`; `port` is ignored and `PORT` is not injected for cronjob apps. Use `private` for databases and internal APIs, and `cronjob` for scheduled batch jobs. See [Apps — Deployment type](/docs/concepts/apps#deployment-type).
- **`schedule`** is required when `deployment_type: cronjob` and ignored otherwise. It's a standard 5-field cron expression (e.g. `0 2 * * *`) or a predefined schedule (`@hourly`, `@daily`, `@weekly`, `@monthly`, `@yearly`, `@midnight`).
- **`PORT` is a reserved env var.** canette sets it automatically on the container from the app's `port` (for `web`/`private` apps) — it cannot be set via `env` or `secrets` in a template, and the API rejects it the same way it does outside of templates.
...
- Any field not recognised as an app-row field (`source_type`, `deployment_type`, `git_url`, etc.) is passed through as `canette.yaml` content and stored as the app's advanced configuration. Unknown fields are silently ignored for forward compatibility.
- **Volumes are not supported in templates.** [Volumes](/docs/configuration/volumes) (PVC / emptyDir / configmap mounts) are always configured after the app exists, on its settings page — there's no `volumes` field in either a template or `canette.yaml`. Add volumes to each app individually after running the wizard.
...
A batch job that runs on a schedule instead of serving traffic. `port` and `env.PORT` don't apply here — cronjob apps never get a `PORT` injected.
...
apps:
  - name: Nightly Report
    slug: nightly-report
    source_type: git
    git_url: https://github.com/org/nightly-report
    git_branch: main
    ...
    secrets:
      - name: SMTP_PASSWORD
        description: "SMTP relay password for sending the report"
```
...
apps:
  ...
  - name: Postgres
    slug: db
    source_type: image
    deployment_type: private   # no public URL — only reachable inside the cluster
    image_url: postgres
    image_tag: "16"
    port: 5432
    secrets:
...
> The `{{apps.db.slug}}` references in `DATABASE_URL` are resolved to the slug you confirm for the Postgres app in the wizard. The Postgres app uses `deployment_type: private` so it has no public URL and is only accessible from other apps inside the cluster.

---

### Full-stack with resource limits and health checks

Demonstrates most template fields at once, including a `cronjob` app alongside the always-on ones.
...
apps:
  ...
  - name: DB Backup
    slug: db-backup
    source_type: git
    git_url: https://github.com/org/db-backup
    git_branch: main
    deployment_type: cronjob
    ...
    secrets:
      - name: BACKUP_BUCKET_KEY
        description: "Write access key for the off-site backup bucket"
    resources:
      requests:
        cpu: 100m
        memory: 128Mi
